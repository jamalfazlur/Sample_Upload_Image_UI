...
class ManageBrand extends Component {
    ...
    componentDidMount() {
        axios.get('https://jamal-api-mysql.herokuapp.com/getlistbrand')
        .then((res) => {
    ...
    onBtnAddClick = () => {
        if(document.getElementById("AddBrandImage").files[0] !== undefined) {
            ...
            axios.post("https://jamal-api-mysql.herokuapp.com/addbrand", formData, headers)
            .then((res) => {
    ...
    onBtnDeleteClick = (id) => {
        if(window.confirm('Are you sure to delete?')) {
            axios.delete('https://jamal-api-mysql.herokuapp.com/deletebrand/' + id)
            .then((res) => {
    ...
    onBtnUpdateClick = (id) => {
        ...
        axios.put("https://jamal-api-mysql.herokuapp.com/editbrand/" + id, formData, headers)
        .then((res) => {
        ...
        var listJSX = this.state.brandList.map((item) => {
            ...
            return (
                <tr>
                    <td>{item.id}</td>
                    <td>{item.nama}</td>
                    <td><img src={`https://jamal-api-mysql.herokuapp.com${item.image}`} alt={item.nama} width={100} /></td>
