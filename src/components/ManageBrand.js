...
class ManageBrand extends Component {
    ...
    componentDidMount() {
        axios.get(`${KONEKSI}/brand/getlistbrand`)
        .then((res) => {
    ...
    onBtnAddClick = () => {
        if(document.getElementById("AddBrandImage").files[0] !== undefined) {
            ...
            axios.post(`${KONEKSI}/brand/addbrand`, formData, headers)
            .then((res) => {
    ...
    onBtnDeleteClick = (id) => {
        if(window.confirm('Are you sure to delete?')) {
            axios.delete(`${KONEKSI}/brand/deletebrand/` + id)
            .then((res) => {
    ...
    onBtnUpdateClick = (id) => {
        ...
        axios.put(`${KONEKSI}/brand/editbrand/` + id, formData, headers)
        .then((res) => {
        ...
        var listJSX = this.state.brandList.map((item) => {
            ...
            return (
                <tr>
                    <td>{item.id}</td>
                    <td>{item.nama}</td>
                    <td><img src={`${KONEKSI}${item.image}`} alt={item.nama} width={100} /></td>
